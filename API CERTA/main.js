...
const serial = async (
    ...
    arduino.pipe(new serialport.ReadlineParser({ delimiter: '\r\n' })).on('data', async (valores) => {
        ...
        const lm35Temperatura1 = parseFloat(valores[5]);
        const lm35Temperatura2 = parseFloat(valores[6]);
        const lm35Temperatura3 = parseFloat(valores[7]);
        const lm35Temperatura4 = parseFloat(valores[8]);
        const lm35Temperatura5 = parseFloat(valores[9]);
        ...
        if (HABILITAR_OPERACAO_INSERIR) {
            if (AMBIENTE == 'producao') {
                ...
                await poolBancoDados.execute(
                    'INSERT INTO sensorumiapi (umi1, umi2, umi3, umi4, umi5) VALUES (?, ?, ?, ?, ?)',
                    [dht11Umidade1, dht11Umidade2, dht11Umidade3, dht11Umidade4, dht11Umidade5]

                );

                await poolBancoDados.execute(
                    'INSERT INTO sensorapi (temp1, temp2, temp3, temp4, temp5) VALUES (?,?,?,?,?) ',
                    [lm35Temperatura1, lm35Temperatura2, lm35Temperatura3, lm35Temperatura4, lm35Temperatura5]
                );
